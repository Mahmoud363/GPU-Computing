...
__global__ void reduce_divergence(double* d_in, int len) {
	__shared__ double Shared_mem[BLOCKS];
	...
	for (int stride = 1; stride < blockDim.x; stride *= 2) {
		__syncthreads();
		if (t % (2 * stride) == 0 && t+stride<BLOCKS) {
	...
	// this method is based on the implementation explained in Nvidia documentation by Mark Harris
...
double  gpu_reduce(double* h_in, int d_in_len)
{
	...
	while (n > 1)
	{
		reduce_divergence << <grid_sz, block_sz >> > (d_in, n);
